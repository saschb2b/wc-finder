...
async function fetchHannoverBusinesses(): Promise<ToiletEntry[]> {
  ...
  for (const el of data.elements) {
    ...
    if (t.fee === 'no' || t['toilets:fee'] === 'no') entryTags.push('kostenlos');
...
async function main() {
  ...
  const output = {
    ...
    source: 'OpenStreetMap - Hannover wheelchair-accessible businesses (restaurants, supermarkets, gyms, bakeries)',
    count: unique.length,
    toilets: unique,
  };
  ...
  const nearby500 = unique.filter(t => dist(t.lat, t.lon, userLat, userLon) < 500);
  const nearby1k = unique.filter(t => dist(t.lat, t.lon, userLat, userLon) < 1000);
  const nearby2k = unique.filter(t => dist(t.lat, t.lon, userLat, userLon) < 2000);
  ...
  console.log(`\nClosest 10:`);
  unique.map(t => ({...t, d: dist(t.lat, t.lon, userLat, userLon)}))
    .sort((a, b) => a.d - b.d)
    .slice(0, 10)
    .forEach(t => {
